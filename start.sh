npx nodemon -w src/server/index.js src/server/index.js &
npx browser-sync start -s -w -f src/client/ --no-open --no-notify --no-ghost-ui --directory --ss ~/Pictures/Screenshots/ &
